...
void APiece::SpawnPiece()
{
    ...
    for (auto&& YZ : YZs) {
        ...
        ABlock* B = GetWorld()->SpawnActor<ABlock>(this->GetActorLocation(), Rotator);
        B->BlockMesh->SetMaterial(1, Colors[ColorIndex]);
        Blocks.Add(B);
        B->AttachToActor(this, FAttachmentTransformRules::KeepRelativeTransform);
        B->SetActorRelativeLocation(FVector(0.0f, YZ.first, YZ.second));
    }
}
...
void APiece::DrawDebugLines()
{
    for (ABlock* B : Blocks)
    {
        ...
        DrawDebugLine(GetWorld(), V1, V2, FColor::Blue, false, 1, 0, 1);
    }
}
...
void APiece::MoveLeft() {
    auto MoveLeftVector = [](FVector OldVector) {
        OldVector.Y -= 10.0f;
        return OldVector;
    };

    if (!CheckWillCollision(MoveLeftVector)) {
...
void APiece::MoveRight() {
    auto MoveRightVector = [](FVector OldVector) {
        OldVector.Y += 10.0f;
        return OldVector;
    };

    if (!CheckWillCollision(MoveRightVector)) {
        FVector NewVector = this->GetActorLocation();
        NewVector.Y += 10.0f;
        SetActorLocation(NewVector);
    }
...
void APiece::MoveDownDraduality()
{
    auto MoveDownVector = [](FVector OldVector) {
        OldVector.Z += 10.0f;
        return OldVector;
    };
}
